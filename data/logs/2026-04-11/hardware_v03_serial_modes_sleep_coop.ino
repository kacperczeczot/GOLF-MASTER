...
NmRunMode nmRunMode = NM_RUN_KEEPALIVE;
...
void handleGatewayNm(uint32_t id, uint8_t *buf, uint8_t len) {
  ...
  if (buf[0] == NM_NODE_SELF && gwNmCmd) {
    ...
    if (nmRunMode == NM_RUN_SLEEP_COOP) {
      if (b1 & NM_MASK_SLEEP) {
        txB1 |= NM_MASK_SLEEP;
      }
...
void processSerial() {
  if (Serial.available() > 0) {
    ...
    if (strcmp(buf, "MODE:KEEPALIVE") == 0) {
      nmRunMode = NM_RUN_KEEPALIVE;
      Serial.println(F("SYS:CAN:NM_MODE_KEEPALIVE"));
      return;
    }
    if (strcmp(buf, "MODE:SLEEP_COOP") == 0) {
      nmRunMode = NM_RUN_SLEEP_COOP;
      Serial.println(F("SYS:CAN:NM_MODE_SLEEP_COOP"));
      return;
    }
...
void loop() {
  ...
  // SLEEP_COOP: krótko po WAKE_END Gateway bywa w przejściu — unikaj fałszywego HANG (tylko jeśli był WAKE_END).
  bool hangSuppressed = (nmRunMode == NM_RUN_SLEEP_COOP && hadWakeEndForGrace && netState != NET_ACTIVE
                         && (millis() - graceStartMs) < GRACE_AFTER_WAKE_END_MS
                         && !isSleepIndicated);
